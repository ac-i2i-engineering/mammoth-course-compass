...
from access_amherst_algo.generate_map import create_map, add_event_markers, generate_heatmap
...
def test_add_event_markers(create_events):
    ...
    marker_count = sum(1 for _ in folium_map._children if isinstance(folium_map._children[_], folium.map.Marker))
...
def test_generate_heatmap(create_events):
    ...
    heatmap_layer = any(isinstance(child, HeatMap) for child in folium_map._children.values())
